...
if (uploaded_data_file is not None) and (uploaded_stress_file is not None):
    ...
    st.write("Patient measurements", pd.concat(patient_measurements))
    st.write("Patient measurements at Hautschnitt and 5 minutes afterwards", pd.concat(measurements_HS_5mins_past))
    st.write("Patient measurements at sections times", pd.concat(measurements_section_times))
